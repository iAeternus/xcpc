/**
 * @see https://www.luogu.com.cn/problem/P3950
 * TODO WA
 */
#include <bits/stdc++.h>
...
std::vector<std::pair<int, int>> vis; // 每次开战的两个部落

int main() {
    int n, m;
    std::cin >> n >> m;
    HLD<int> hld(n, std::vector<int>(n, 0));
    for (int i = 1; i < n; ++i) {
        int p, q;
        std::cin >> p >> q;
        hld.addEdge(p, q);
    ...
    while (m--) {
        ...
        if (op == 'Q') {
            std::cin >> p >> q;
            std::cout << (hld.queryPath(p, q) == 0 ? "Yes" : "No") << std::endl;
        } else if (op == 'C') {
            std::cin >> p >> q;
            vis.push_back({p, q});
            hld.updatePath(p, q, 1);
        } else {
            std::cin >> x;
            auto [p, q] = vis[x - 1];
            hld.updatePath(p, q, -1);
